...
- The base of Exact-Ti Security Platform, generated by running `yarn build --linux --skip-os-packages --release` or `yarn build --linux-arm --skip-os-packages --release` for arm packages
- The build of each plugin in `exactti-dashboard-plugins` repo
- The build of the `wazuh-security-dashboards-plugin` repo
- The build of the `exactti-dashboard-reporting ` repo
- The build of the `exactti-dashboard-security-analytics ` repo
...
- `-a`, `--app`: URL or path to the zip that contains the `exactti-dashboard-plugins` plugins build.
- `-b`, `--base`: URL or path to the zip that contains the `exactti-dashboard build`.
- `-s`, `--security`: URL or path to the zip that contains the `wazuh-security-dashboards-plugin` build.
- `-rp`, `--reportPlugin`: URL or path to the zip that contains the `exactti-dashboard-reporting` build.
- `-sa`, `--securityAnalytics`: URL or path to the zip that contains the `exactti-dashboard-security-analytics` build.
...
> [!WARNING]
> To build `arm` packages, you need to run the script in an arm machine, and use an arm build of the exactti-dashboard base with `-b`
