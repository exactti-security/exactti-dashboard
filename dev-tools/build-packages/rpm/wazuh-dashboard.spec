...
# Foundation.
Summary:     Exact-Ti dashboard is a user interface and visualization tool for security-related data. Documentation can be found at https://documentation.exactti.com/current/getting-started/components/exactti-dashboard.html
Name:        exactti-dashboard
Version:     %{_version}
Release:     %{_release}
License:     GPL
...
buildroot:   %{_tmppath}/%{name}-%{version}-%{release}-exactti-dashboard-%(%{__id_u} -n)
...
%global DASHBOARD_FILE exactti-dashboard.tar.gz
...
%description
Exact-Ti dashboard is a user interface and visualization tool for security-related data. This Exact-Ti central component enables exploring, visualizing, and analyzing the stored security alerts generated by the Exact-Ti server. Exact-Ti dashboard enables inspecting the status and managing the configurations of the Exact-Ti cluster and agents as well as creating and managing users and roles. In addition, it allows testing the ruleset and making calls to the Exact-Ti API. Documentation can be found at https://documentation.exactti.com/current/getting-started/components/exactti-dashboard.html
...
sed -i 's/OSD_NODE_OPTS_PREFIX/OSD_PATH_CONF="\/etc\/exactti-dashboard" OSD_NODE_OPTS_PREFIX/g' "exactti-dashboard-base/bin/opensearch-dashboards"
sed -i 's/OSD_USE_NODE_JS_FILE_PATH/OSD_PATH_CONF="\/etc\/exactti-dashboard" OSD_USE_NODE_JS_FILE_PATH/g' "exactti-dashboard-base/bin/opensearch-dashboards-keystore"
...
mv exactti-dashboard-base/config/node.options %{buildroot}%{CONFIG_DIR}
mv exactti-dashboard-base/config/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp exactti-dashboard-base/VERSION.json %{buildroot}%{INSTALL_DIR}

mv exactti-dashboard-base/* %{buildroot}%{INSTALL_DIR}

# Set custom welcome styles

mkdir -p %{buildroot}%{INSTALL_DIR}/config

cp %{buildroot}%{INSTALL_DIR}/etc/services/exactti-dashboard.service %{buildroot}/usr/lib/systemd/system/exactti-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/exactti-dashboard

chmod 644 %{buildroot}/usr/lib/systemd/system/exactti-dashboard.service
chmod 640 %{buildroot}/etc/default/exactti-dashboard
...
chown root:root %{buildroot}/usr/lib/systemd/system/exactti-dashboard.service
...
%pre
# Create the exactti-dashboard group if it doesn't exists
if [ $1 = 1 ]; then
  ...
  # Create the exactti-dashboard user if it doesn't exists
  if ! getent passwd %{USER} > /dev/null 2>&1; then
    useradd -g %{GROUP} -G %{USER} -d %{INSTALL_DIR}/ -r -s /sbin/nologin exactti-dashboard
  fi
fi
# Stop the services to upgrade the package
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet exactti-dashboard > /dev/null 2>&1; then
    systemctl stop exactti-dashboard.service > /dev/null 2>&1
    touch %{INSTALL_DIR}/exactti-dashboard.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service exactti-dashboard status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service exactti-dashboard stop > /dev/null 2>&1
    touch %{INSTALL_DIR}/exactti-dashboard.restart
  fi
fi
...
rm -f /usr/share/exactti-dashboard/VERSION
...
%preun
if [ $1 = 0 ];then # Remove
  echo -n "Stopping exactti-dashboard service..."
...
if [ $1 = 0 ];then
  ...
  # Remove the exactti-dashboard user if it exists
  if getent passwd %{USER} > /dev/null 2>&1; then
    userdel %{USER} >/dev/null 2>&1
  fi
  # Remove the exactti-dashboard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group %{GROUP} > /dev/null 2>&1; then
  ...
  # Remove /etc/exactti-dashboard and /usr/share/exactti-dashboard dirs
  rm -rf %{INSTALL_DIR}
  if [ -d %{PID_DIR} ]; then
    rm -rf %{PID_DIR}
  fi
...
if [ -f %{INSTALL_DIR}/exactti-dashboard.restart ]; then
  rm -f %{INSTALL_DIR}/exactti-dashboard.restart
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1; then
    systemctl restart exactti-dashboard.service > /dev/null 2>&1
  # Check for SysV
  elif command -v service > /dev/null 2>&1; then
    service exactti-dashboard restart > /dev/null 2>&1
  fi
...
%config(noreplace) %attr(0750, %{USER}, %{GROUP}) "/etc/default/exactti-dashboard"
...
%attr(644, root, root) "/usr/lib/systemd/system/exactti-dashboard.service"

%changelog
* Sat Apr 11 2026 support <info@wazuh.com> - 4.14.5
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-5.html
...
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-4.html
* Wed Feb 11 2026 support <info@wazuh.com> - 4.14.3
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-3.html
* Wed Jan 14 2026 support <info@wazuh.com> - 4.14.2
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-2.html
* Wed Nov 12 2025 support <info@wazuh.com> - 4.14.1
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-1.html
* Wed Oct 22 2025 support <info@wazuh.com> - 4.14.0
- More info: https://documentation.exactti.com/current/release-notes/release-4-14-0.html
* Wed Sep 24 2025 support <info@wazuh.com> - 4.13.1
- More info: https://documentation.exactti.com/current/release-notes/release-4-13-1.html
* Thu Sep 18 2025 support <info@wazuh.com> - 4.13.0
- More info: https://documentation.exactti.com/current/release-notes/release-4-13-0.html
* Wed May 21 2025 support <info@wazuh.com> - 4.10.2
- More info: https://documentation.exactti.com/current/release-notes/release-4-10-2.html
* Wed May 07 2025 support <info@wazuh.com> - 4.12.0
- More info: https://documentation.exactti.com/current/release-notes/release-4-12-0.html
* Tue Apr 01 2025 support <info@wazuh.com> - 4.11.2
- More info: https://documentation.exactti.com/current/release-notes/release-4-11-2.html
* Wed Mar 12 2025 support <info@wazuh.com> - 4.11.1
- More info: https://documentation.exactti.com/current/release-notes/release-4-11-1.html
* Fri Feb 21 2025 support <info@wazuh.com> - 4.11.0
- More info: https://documentation.exactti.com/current/release-notes/release-4-11-0.html
* Thu Jan 16 2025 support <info@wazuh.com> - 4.10.1
- More info: https://documentation.exactti.com/current/release-notes/release-4-10-1.html
...
- More info: https://documentation.exactti.com/current/release-notes/release-4-9-2.html
* Tue Oct 15 2024 support <info@wazuh.com> - 4.9.1
- More info: https://documentation.exactti.com/current/release-notes/release-4-9-1.html
